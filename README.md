...
- **Original audio, never auto-dubs** — YouTube encodes its machine-dubbed tracks at a higher bitrate than the original, so the tool filters to the original-language track before picking quality
...
**The YouTube video came out in the wrong voice or language — why?**
It won't any more. YouTube attaches machine-generated dubbed audio tracks to many videos and
encodes them at a *higher* bitrate than the original, so a plain "best audio" pick lands on the
dub. The tool now filters to the original-language track first and only then picks the best
quality, printing which track it chose:

```
Audio track     :  en (original) — auto-dubs skipped
```

Videos without dubs are unaffected.
